...
class StateGame(State):
    ...
    def _evaluateTilePos2(self):
        "My previous tile pos was incorrect, so new one it is"
        # If another tile got touched

        if int(self.player.rect.x/16) > self.levelProgress:
            self.levelProgress += 1
            self._loadSpriteColumn2(self.levelMatrix[:, self.levelProgress], self.player.rect.x)
    ...
    def _borderHandling(self, leftinput, rightinput):
        ...
        print(self.tilePos, self.player.rect.x%32,(self.player.rect.x))
    ...
    def _checkCollisions(self):
        ...
        """
        move = 0
        col_list = self.player.checkCollisions_block()
                ...
                # Else, player collided with left side
                    ...
                    if self.player.rect.left < collided.rect.right:
                        ...
                        if abs(collided.rect.right - self.player.rect.left) > abs(move):
                            move = collided.rect.right - self.player.rect.left
    ...
    def _borderHandling2(self, leftinput, rightinput):
        """
        This method deals with the logic that when peach is near a border, the level should not move anymore.
        Most of the time, peach stays at 1 x-position and the level moves.
        However, when approaching a border, if the level moved too, then the game would show the part where
        ...
        if self.levelMoving:
            if rightinput:
                ...
                if adjusted_pos != 0:
                    for sprite in self.all_sprites.sprites():
                        sprite.moveRight(adjusted_pos)

                self._evaluateTilePos()
                #todo: only up to here checked
        ...
        # Check if new level column has to be loaded
        self._evaluateTilePos2()

        print(self.tilePos, self.player.rect.x%32,(self.player.rect.x))



    def _loadSpriteColumn2(self, column, playerX):
        """
        ...
        for row in range(13):
            ...
            if column[row] != 0:
                ...
                if column[row] == 1:  # If block
                    ...
                    new_sprite = SpriteBlock(row * 16 * 2, playerX + 32 * 23)
