...
class SpritePlayer(pygame.sprite.Sprite):
    ...
    def enforceNoVerticalClipping(self):
        ...
        if len(col_list) == 0:
            ...
            # todo: better: Find out which sprite is the highest and only do it for this sprite
            for collided in col_list:

                #todo: needs a check of ????????????

                """
                Before it goes up, it collides with 3 sprites.
                Then only with 1
                """

                if collided.rect.top >= self.rect.top:
                    #break
    ...
    def applyGravity(self):
        # terminal velocity
        print("current bottomLocation is:",self.rect.bottom)
